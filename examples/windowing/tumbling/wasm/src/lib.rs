//! Tumbling windowing demo as a WebAssembly processor.
//!
//! The wasm half of `examples/windowing/tumbling/`: receives the merged rows
//! of both NATS sources (one batch per source per stream item), keeps open
//! windows in its checkpoint state, and emits one `WindowTotal` row per
//! closed `(window_id, symbol)` group — Beam's default trigger, where a
//! window closes when the event-time watermark passes its end. The geometry
//! comes from the `window.*` config keys the host injects from the KDL
//! `window` block; the plugin counterpart in
//! `examples/windowing/tumbling/plugin` runs the same logic as a native
//! plugin.
//!
//! # Build
//!
...
//! `target/wasm32-wasip2/release/windowing_tumbling_wasm.wasm`.
...
pub fn build() -> Pipeline {
    let mut pipeline = Pipeline::new("windowing_tumbling");
    pipeline
        .data
...
#[cfg(target_arch = "wasm32")]
saci_processor::export_pipeline!(build, state = WindowState);
